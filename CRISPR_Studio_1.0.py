...
# The function spacerExtract extract the spacer information contained
...
def spacerExtract(inFile):
    ...
    '''
        ...
        output:
            ...
            - isolateList -> list of all the isolate
...
def checkSpacer(spacerDict, averageLength, outFasta):
    '''
        ...
        input:
            ...
            averageLength -> avarageLength of the spacers
            outFasta -> file where the sequence of all the spacers will be writen in fasta format
...
def extractMatch(alnFile, cutoff, spacerDict):
    ...
    with open(alnFile, 'rU') as bl:
        for line in bl:
            ...
            if  score <= cutoff and query != subject: # if score smaller that cutoff and if not vs self
                outFile.write('%s\t%s\n' % (query, subject))
...
def attributeClsColor(mclFile, spacerDict):
    ...
    with open(mclFile, 'rU') as fl:
        for line in fl:
            cl += 1
            clName = 'cls_%04d' % cl ## mettre 5 digits
            array = line.strip().split()
...
def main():
    ...
    parser.add_argument("-g", '--grayOut', dest='gray', help='The uniq spacers will be grayed (Optional: by default, CRISPR_Studio attribute a uniq color each group of spacer.)', action='store_true')
    ...
    parser.add_argument("-s", '--sort', dest='sort', help='The order of the isolates in the figure: Available option: CRISPRDetect, HClust, File prividing a list of the isolates in the desired order. CRISPRDetect = order in the gff file. HClust = Order based on hiearchical clustering of a distance matrix. File = Order privided in a single column file with the isolates names as the should appear in the figure. The option -l override this option (Default: CRISPRDetect)')
